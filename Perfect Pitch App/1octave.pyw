...
# This font was chosen by LeMaster Tech - the author of the GUI which I have taken to edit for my own purposes
...
one_octave = pl.one_octave
one_octavewhites = pl.one_octavewhites
one_octaveblacks = pl.one_octaveblacks

# Appends each note in the given octave to the necessary array.

for i in range(len(one_octavewhites)):
    white_sounds.append(mixer.Sound(f'assets//notes//{one_octavewhites[i]}.wav'))

for i in range(len(one_octaveblacks)):
    black_sounds.append(mixer.Sound(f'assets//notes//{one_octaveblacks[i]}.wav'))

for i in range(len(one_octave)):
    piano_sounds.append(mixer.Sound(f'assets//notes//{one_octave[i]}.wav'))
...
def draw_piano(whites, blacks):
    ...
    piano_width = 7 * 35 # 7 for the 7 white keys and 35 for number of pixels wide each white key is
    piano_x = (WIDTH - piano_width) // 2 # x-position to start drawing the piano
    piano_y = 250  # y-position to start drawing the piano

    # Draw white keys
    for i in range(7): #Draws 7 rectangles for the 7 independent white keys in an octave
        ...
        key_label = small_font.render(one_octavewhites[i], True, 'black')
    ...
    # Draw black keys
    for i in range(5): #5 for the 5 black keys in an octave
        ...
        key_label = real_small_font.render(one_octaveblacks[i], True, 'white')
        screen.blit(key_label, (piano_x + 25 + (i * 35) + (skip_count * 35), piano_y + 180))
        black_rects.append(black_rect)

        # Handles skips between each black key
        skip_track += 1
    ...
    # Return the lists of white and black rectangles along with modified lists of active white and black keys
...
random_position = random.randint(0, len(one_octave) - 1)
pygame.mixer.Sound.play(piano_sounds[random_position])
randomNote = one_octave[random_position]

while run:
    timer.tick(fps)
    screen.fill('teal')
    
    #Functions must be called after the background colour.
    # Otherwise the colour will be in the foreground and everything will be covered.
    ...
    if game_state == 'playing':
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
                # Stops running if the app is closed

            if event.type == pygame.MOUSEBUTTONDOWN:
                #If the mouse is pressed... 
                if replay_button_rect.collidepoint(event.pos):
                        # In the boundaries of the replay button
                        pygame.mixer.Sound.play(piano_sounds[random_position])
                        # play the sound file found at the randomly generated position
                ...
                if note_presses < 3:
                        ...
                        for i in range(len(black_keys)):
                            if black_keys[i].collidepoint(event.pos): # If a click is detected on any black key.
                                ...
                                note_pressed = one_octaveblacks[i] # Stores the name of the key pressed to compare
                        ...
                        for i in range(len(white_keys)):
                            if white_keys[i].collidepoint(event.pos) and not black_key: 
                                ...
                                note_pressed = one_octavewhites[i] # Stores the name of the key pressed to compare
                                note_presses += 1 # Adds 1 to the number of notes pressed to limit the user to 3
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if restart_button_rect.collidepoint(event.pos):
                    ...
                    randomNum = random.randint(0, len(one_octave) - 1)
                    randomNote = one_octave[randomNum]
