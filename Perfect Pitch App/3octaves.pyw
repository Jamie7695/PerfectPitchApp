...
three_octave = pl.three_octave
three_octavewhites = pl.three_octavewhites
three_octaveblacks = pl.three_octaveblacks

# Appends each note in the given octave to the necessary array.

for i in range(len(three_octavewhites)):
    white_sounds.append(mixer.Sound(f'assets\\notes\\{three_octavewhites[i]}.wav'))

for i in range(len(three_octaveblacks)):
    black_sounds.append(mixer.Sound(f'assets\\notes\\{three_octaveblacks[i]}.wav'))

for i in range(len(three_octave)):
    piano_sounds.append(mixer.Sound(f'assets\\notes\\{three_octave[i]}.wav'))
...
def draw_piano(whites, blacks):
    ...
    piano_width = 21 * 35 # 21 for the 21 white keys and 35 for number of pixels wide each white key is
    piano_x = (WIDTH - piano_width) // 2 # x-position to start drawing the piano
    piano_y = 250  # y-position to start drawing the piano
    
    # Draw white keys
    for i in range(21): # Draws 21 rectangles for the 21 independent white keys in three octaves
        ...
        key_label = small_font.render(three_octavewhites[i], True, 'black')
    ...
    # Draw black keys
    for i in range(15): # 15 for the 15 black keys in three octaves
        ...
        key_label = real_small_font.render(three_octaveblacks[i], True, 'white')
...
randomNum = random.randint(0, len(three_octave) - 1)
pygame.mixer.Sound.play(piano_sounds[randomNum])
randomNote = three_octave[randomNum]
                ...
                if note_presses < 3:
                    ...
                    for i in range(len(black_keys)):
                        if black_keys[i].collidepoint(event.pos): # If a click is detected on any black key.
                            ...
                            note_pressed = three_octaveblacks[i] # Stores the name of the key pressed to compare
                    ...
                    for i in range(len(white_keys)):
                        if white_keys[i].collidepoint(event.pos) and not black_key: 
                            ...
                            note_pressed = three_octavewhites[i] # Stores the name of the key pressed to compare
                            note_presses += 1 # Adds 1 to the number of notes pressed to limit the user to 3
